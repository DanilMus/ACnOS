...
std::ofstream outputFile("task5.txt");
...
void writerThread(int id) {
    for (int i = 0; i < 5; ++i) {
        ...
        outputFile << buffer[writeIndex] << std::endl;
...
void readerThread() {
    for (int i = 0; i < 15; ++i) {  // При чтении, каждый читатель считывает все записи из буфера
        std::unique_lock<std::mutex> lock(mtx);
        ...
        std::cout << "Reader: " << buffer[readIndex] << std::endl;
